...
namespace citrus {
	...
	std::vector<VkImageView> frameStore::getViews(uint32_t const& index) {
		return { frames[index].color.view, frames[index].index.view, frames[index].depth.view };
	}
	std::vector<VkAttachmentDescription> frameStore::getAttachmentDescriptions(bool transitionToRead) {
	...
	std::vector<VkAttachmentReference> frameStore::getColorAttachmentRefs() {
	...
	void buffer::flushRange(uint64_t start, uint64_t size) {
		...
		range.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
		range.memory = mem;
		range.offset = roundDownAlign(start, inst->flushAlign);
		range.pNext = nullptr;
		range.size = roundUpAlign(size, inst->flushAlign);
		if (vkFlushMappedMemoryRanges(inst->_device, 1, &range) != VK_SUCCESS) throw std::runtime_error("failed to map flush memory range");
	}
	...
	void cubemapStore::addImage(image4b const& data) {
		...
		res.offset = roundUpAlign(nextAvailable, align);
	...
	cubemapStore::cubemapStore(instance* inst, std::vector<fpath> const& paths) : cubemapStore(inst) {
