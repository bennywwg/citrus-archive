...
namespace citrus {
	...
	public:
		...
		cubemapStore(instance* inst, std::vector<fpath> const& paths);
		~cubemapStore();
	};
	...
	public:
		...
		std::vector<VkImageView>		getViews(uint32_t const& index);
		std::vector<VkAttachmentDescription> getAttachmentDescriptions(bool transitionToRead);
		std::vector<VkAttachmentReference> getColorAttachmentRefs();
