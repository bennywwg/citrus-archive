...
namespace citrus {
	void renderSystem::loadTextures() {
		...
		for (int i = 0; i < texturePaths.size(); i++) {
			...
			addr = roundUpAlign(addr, memRequirements.alignment);
			textures[i].off = addr;
	...
	void renderSystem::freeTextures() {
	...
	void renderSystem::collectModelInfo() {
	...
	void renderSystem::loadModels() {
		...
		std::cout << "citrus::renderSystem::loadModels(...): " + std::to_string(vertexAddr) + " bytes of vertex memory will be consumed\n";
		//util::sout("citrus::renderSystem::loadModels(...): " + std::to_string(vertexAddr) + " bytes of vertex memory will be consumed\n");
	}
	void renderSystem::initializeModelData() {
	...
	void renderSystem::loadAnimations() {
		...
		for (int i = 0; i < animations.size(); i++) {
			for (int j = 0; j < models.size(); j++) {
				...
				if (success)
					std::cout << "animation bound: model " + std::to_string(j) + " lod 0, ani " + std::to_string(i) + "\n";
					//util::sout("animation bound: model " + std::to_string(j) + " lod 0, ani " + std::to_string(i) + "\n");
			}
		}
	}

	void renderSystem::initializeThreads(uint32_t threadCount) {
		...
		for (size_t t = 1; t < renderThreads.size(); t++) {
			renderThreads[t] = std::thread(&renderSystem::renderFunc, this, t);
		}
	}
	void renderSystem::freeThreads() {
	...
	/*void renderSystem::sequence(vector<vector<itemInfo>> const& items, vector<vector<itemDrawRange>> & ranges) {
		int totalItems = 0;
	...
	void renderSystem::renderFunc(uint32_t threadIndex) {
	...
	bool renderSystem::renderDone() const {
		for (int i = 0; i < renderThreads.size(); i++) {
			if (renderGo[i] == true) return false;
		}
		return true;
	}
	void renderSystem::postProcess(int frameIndex, int windowSwapIndex, std::vector<VkSemaphore> waits, VkSemaphore signal) {
	...
	renderSystem::renderSystem(instance& vkinst, fpath texturePath, fpath modelPath, fpath animationPath) : inst(vkinst), renderGo(4), cubemaps(&inst) {
		texturePaths = filesInDirectory(texturePath, ".png");
		modelPaths = filesInDirectory(modelPath, ".dae");
		animationPaths = filesInDirectory(animationPath, ".cta");

		cubemaps.addImage(image4b((texturePath / "cubemaps" / "sky.png").string()));

		std::cout << "Render System:\n";
		std::cout << "  Models: " + std::to_string(modelPaths.size()) + "\n";
		for (int i = 0; i < modelPaths.size(); i++) {
			std::cout << "    " + std::to_string(i) + ": " + modelPaths[i].relative_path().string() + "\n";
		}
		std::cout << "  Textures: " + std::to_string(texturePaths.size()) + "\n";
		for (int i = 0; i < texturePaths.size(); i++) {
			std::cout << "    " + std::to_string(i) + ": " + texturePaths[i].relative_path().string() + "\n";
		}
		std::cout << "  Animations: " + std::to_string(animationPaths.size()) + "\n";
		for (int i = 0; i < animationPaths.size(); i++) {
			std::cout << "    " + std::to_string(i) + ": " + animationPaths[i].relative_path().string() + "\n";
		}
