...
namespace citrus {
	void meshPass::initializeDescriptors() {
		...
		{
			std::vector<VkDescriptorImageInfo> cubeInfos;
    ...
    void meshPass::initializeRenderPass() {
		std::vector<VkAttachmentDescription> attachments = frame->getAttachmentDescriptions(transitionToRead);
		std::vector<VkAttachmentReference> colorRefs = frame->getColorAttachmentRefs();
	...
	void meshPass::mapModels() {
		...
		std::cout << "Mesh Pass (";
		std::cout << rigged ? "rigged " : "";
		std::cout << "):\n";
		std::cout << "  Mapped Models: " + std::to_string(mappings.size()) + "\n";
		for (int i = 0; i < mappings.size(); i++) {
			std::cout << "    " + std::to_string(i) + ": " + sys.models[mappings[i].modelIndex].source.relative_path().string() + "\n";
		}
	}
	...
	void meshPass::renderPartial(uint32_t const & threadIndex) {
		...
		std::vector<uint64_t> boneOffsets;
		if (threadIndex == 3)
			boneOffsets = { };
		if (rigged) {
			...
			uint64_t boneOffset = roundUpAlign(availableForMyThread * (uint64_t) threadIndex, ssbos[sys.frameIndex].align);
			...
			for (uint32_t i = ranges[threadIndex].begin; i < ranges[threadIndex].end; i++) {
				...
				boneOffset = roundUpAlign(boneOffset, ssbos[sys.frameIndex].align);
			}
		...
		for(uint32_t i = ranges[threadIndex].begin; i < ranges[threadIndex].end; i++) {
			if (items[i].enabled && (!cullObscured || sys.frameCull.testSphere(items[i].pos, sys.models[mappings[items[i].modelIndex].modelIndex].radius))) {
				...
				copyMat4x3ToRowMajor(modTmp, pushData.rowMajorModel);
	...
	meshPass::meshPass(renderSystem & sys, frameStore* fstore, bool textured, bool lit, bool rigged, fpath const& vertLoc, fpath const& fragLoc, bool transitionToRead) :
		sysNode(sys),
		frame(fstore) {
		...
		std::map<meshAttributeUsage, uint32_t> locMap;
