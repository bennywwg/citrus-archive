...
namespace citrus {
	...
	void boneContainer::loadAllBones(const aiScene* scene, const nodeContainer& nodes) {
		...
		for (size_t j = 0; j < mesh->mNumBones; j++) {
			...
			std::string name = inBone.mName.C_Str();
	...
	meshMemoryStructure mesh::getDescription(uint64_t vertStart, uint64_t vertAlign, uint64_t indexStart, uint64_t indexAlign) const {
		...
		std::vector<meshAttributeUsage> usages;
		...
		for (int i = 0; i < usages.size(); i++) {
			vertStart = roundUpAlign(vertStart, vertAlign);
		...
		return meshMemoryStructure {
			...
			offsets,																		// vert starts
			vertStart,																		// vert end
			roundUpAlign(indexStart, indexAlign),										// index start
			roundUpAlign(indexStart, indexAlign) + index.size() * sizeof(uint16_t),	// index end
			pos.size(),																		// vert count
	...
	void mesh::calculateAnimationTransforms(int animationIndex, std::vector<glm::mat4> & data, double time, behavior mode) const {
		...
		if (mode == behavior::repeat) time = wrap(time, ani.ani->begin, ani.ani->end);
	...
	std::string meshAttributeUtils::toString(meshAttributeUsage usage) {
		if (usage == meshAttributeUsage::positionType) {
			return "position";
		}
		else if (usage == meshAttributeUsage::normalType) {
	...
	std::string meshAttributeUtils::getAttribTypeName(meshAttributeUsage usage) {
		if (usage == meshAttributeUsage::positionType) {
			return "vec3";
		} else if (usage == meshAttributeUsage::normalType) {
			return "vec3";
	...
	meshUsageLocationMapping::meshUsageLocationMapping(std::map<meshAttributeUsage, uint32_t> mapping) {
