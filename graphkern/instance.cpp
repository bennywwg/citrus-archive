...
namespace citrus {
	...
	void instance::initInstance(std::string name) {
	...
	void instance::checkExtensions() {
		...
		for (const char* extensionName : required) {
			...
			if (!found) throw std::runtime_error(("Extension \"" + std::string(extensionName) + "\" not available").c_str());
		}
	}
	...
	void instance::checkValidationLayers() {
		for (const char* layerName : getRequiredLayers()) {
			...
			if (!found) throw std::runtime_error(("Layer \"" + std::string(layerName) + "\" not available").c_str());
		}
	}
	...
	void instance::loadPhysicalDevices() {
		...
		_availableDevices = std::vector<VkPhysicalDevice>(deviceCount);
	...
	void instance::waitForFences(std::vector<VkFence>const& fens) {
	...
	uint64_t instance::minStorageBufferOffsetAlignment() {
		VkPhysicalDeviceProperties props;
		vkGetPhysicalDeviceProperties(_chosenDevice, &props);
