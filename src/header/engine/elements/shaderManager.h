	...
	public:
		...
		inline void loadSimpleShader(std::string vertLoc, std::string fragLoc) {
			try {
				...
				eng()->Log("Loaded Shader \"" + vertLoc + "\" with index " + std::to_string(index));
			} catch(const std::runtime_error& er) {
				eng()->Log("Failed to load simple shader \"" + vertLoc + "\": " + er.what());
			} catch(...) {
				eng()->Log("Failed to load simple shader \"" + vertLoc + "\": (Whack error)");
			}
		}
